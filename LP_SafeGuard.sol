...
contract DepositContract {
    ...
    constructor(address _tokenA, address _tokenB, address _lpToken, uint256 _tokenA_amount, uint256 _tokenB_amount, uint256 _total_tokenA_threshold, address _liquidityPool) {
    ...
    function deposit(uint256 _tokenA_amount, uint256 _tokenB_amount) public {
        require(tokenA.transferFrom(msg.sender, address(this), _tokenA_amount), "Transfer of tokenA failed");
        require(tokenB.transferFrom(msg.sender, address(this), _tokenB_amount), "Transfer of tokenB failed");
        ...
        if(_tokenA_amount / _tokenB_amount == tokenA_amount / tokenB_amount) {
            ...
            if(total_tokenA_deposited >= total_tokenA_threshold && deadline == 0) {
    ...
    function reclaim() public {
        ...
        require(tokenA.transfer(msg.sender, depositors[msg.sender].tokenA_deposited), "Transfer of tokenA failed");
        require(tokenB.transfer(msg.sender, depositors[msg.sender].tokenB_deposited), "Transfer of tokenB failed");
    ...
    function depositToLiquidityPool() public {
        require(total_tokenA_deposited >= total_tokenA_threshold, "Threshold not met");
        ...
        for (uint i = 0; i < depositors.length; i++) {
            ...
            if (tokenA_to_deposit / tokenB_to_deposit == tokenA_amount / tokenB_amount) {
                ...
                require(tokenA.approve(liquidityPool, tokenA_to_deposit), "Approval of tokenA failed");
                require(tokenB.approve(liquidityPool, tokenB_to_deposit), "Approval of tokenB failed");

                // Call the addLiquidity function of the liquidity pool
                (bool success,) = liquidityPool.call(abi.encodeWithSignature("addLiquidity(address,address,uint256,uint256)", address(tokenA), address(tokenB), tokenA_to_deposit, tokenB_to_deposit));
    ...
    function claimLP() public {
        ...
        require(lpToken.transfer(msg.sender, lp_tokens), "Transfer of LP tokens failed");
    }
}
